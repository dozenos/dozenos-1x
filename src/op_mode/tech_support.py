...
def _get_uptime():
    from dozenos.utils.system import get_uptime_seconds

    return get_uptime_seconds()

def _get_load_average():
    from dozenos.utils.system import get_load_averages
...
def _get_storage():
    from dozenos.utils.disk import get_persistent_storage_stats
...
def _get_running_config():
    ...
    from dozenos.utils.strip_config import strip_config_source
...
def _get_nic_data():
    from dozenos.utils.process import ip_cmd
...
def _get_image_info():
    from dozenos.system.image import get_images_details

    return get_images_details()

def _get_kernel_modules():
    from dozenos.utils.kernel import lsmod
...
def _get_raw_data():
    ...
    # DozenOS-specific information
    data["dozenos"] = {}

    ## The equivalent of "show version"
    from dozenos.version import get_version_data
    data["dozenos"]["version"] = _get_version_data()

    ## Installed images
    data["dozenos"]["images"] = _get_image_info()
    ...
    data["dozenos"]["config"]["running"] = _get_running_config()
    ...
    data["dozenos"]["config"]["boot"] = _get_boot_config()

    ## Config scripts
    data["dozenos"]["config"]["scripts"] = _get_config_scripts()
...
def show(raw: bool):
    ...
    if raw:
        ...
        raise dozenos.opmode.UnsupportedOperation("Formatted output is not implemented yet")

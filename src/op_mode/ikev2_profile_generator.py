...
from dozenos.config import config_dict_mangle_acme
from dozenos.pki import CERT_BEGIN
from dozenos.pki import CERT_END
...
# DozenOS comes with a multitude of different proposals for a connection.
#
# We take all available proposals from the DozenOS CLI and ask the user which one
...
profile_name = 'DozenOS IKEv2 Profile'
if args.profile:
    profile_name = args.profile

vpn_name = 'DozenOS IKEv2 VPN'
if args.name:
    vpn_name = args.name
...
dozenos2client_cipher = dozenos2apple_cipher if args.os == 'ios' else dozenos2windows_cipher;
dozenos2client_integrity = dozenos2apple_integrity if args.os == 'ios' else dozenos2windows_integrity;
dozenos2client_dh_group = dozenos2apple_dh_group if args.os == 'ios' else dozenos2windows_dh_group
...
def transform_pfs(pfs, ike_dh_group):
    ...
    if args.os == 'ios':
        if pfs_enabled:
            if pfs_dh_group not in set(dozenos2apple_dh_group):
        ...
        if pfs_enabled:
            if pfs_dh_group not in set(dozenos2windows_pfs_group):
                exit(f'The PFS group configured for "{args.connection}" is not supported by the client!')
            return dozenos2windows_pfs_group[ pfs_dh_group ]
        else:
            return 'None'
...
for _, proposal in ike_proposal.items():
    if {'dh_group', 'encryption', 'hash'} <= set(proposal):
        if (proposal['encryption'] in set(dozenos2client_cipher) and
            proposal['hash'] in set(dozenos2client_integrity) and
            proposal['dh_group'] in set(dozenos2client_dh_group)):

            # We 're-code' from the DozenOS IPsec proposals to the Apple naming scheme
            proposal['encryption'] = dozenos2client_cipher[ proposal['encryption'] ]
            proposal['hash'] = dozenos2client_integrity[ proposal['hash'] ]
...
for _, proposal in esp_group['proposal'].items():
    if {'encryption', 'hash'} <= set(proposal):
        if proposal['encryption'] in set(dozenos2client_cipher) and proposal['hash'] in set(dozenos2client_integrity):
            # We 're-code' from the DozenOS IPsec proposals to the Apple naming scheme
            proposal['encryption'] = dozenos2client_cipher[ proposal['encryption'] ]
            proposal['hash'] = dozenos2client_integrity[ proposal['hash'] ]
...
data['ike_encryption']['dh_group'] = dozenos2client_dh_group[ data['ike_encryption']['dh_group'] ]

print('\n\n==== <snip> ====')
if args.os == 'ios':
    print(render_to_string('ipsec/ios_profile.j2', data))
    print('==== </snip> ====\n')
    print('Save the XML from above to a new file named "dozenos.mobileconfig" and E-Mail it to your phone.')

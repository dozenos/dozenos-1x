...
from dozenos.kea import kea_get_active_config
from dozenos.kea import kea_get_dhcp_pools
from dozenos.kea import kea_get_leases
from dozenos.kea import kea_get_server_leases
from dozenos.kea import kea_get_static_mappings
...
def _verify_client(func):
    ...
    def _wrapper(*args, **kwargs):
        ...
        if not config_exists:
            raise dozenos.opmode.UnconfiguredObject(unconf_message)
...
def show_server_pool_statistics(
    ...
    try:
        ...
        if vrf:
            raise dozenos.opmode.DataUnavailable(
                f'Cannot fetch DHCP server configuration for VRF {vrf}'
            )
        else:
            raise dozenos.opmode.DataUnavailable('Cannot fetch DHCP server configuration')
    ...
    if pool and active_pools and pool not in active_pools:
        if vrf:
            ...
            raise dozenos.opmode.IncorrectValue(f'DHCP{v} pool "{pool}" does not exist!')
...
def show_server_leases(
    ...
    if sorted and sorted not in sort_valid:
        raise dozenos.opmode.IncorrectValue(f'DHCP{v} sort "{sorted}" is invalid!')

    if state and state not in lease_valid_states:
        raise dozenos.opmode.IncorrectValue(f'DHCP{v} state "{state}" is invalid!')

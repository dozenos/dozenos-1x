...
from dozenos.pki import encode_public_key
from dozenos.pki import encode_private_key
from dozenos.pki import encode_dh_parameters
from dozenos.pki import find_chain
from dozenos.pki import get_certificate_fingerprint
...
from dozenos.pki import verify_certificate
        ...
        def _wrapper(*args, **kwargs):
            ...
            if name:
                ...
                if not exists and target == 'ca':
                    from dozenos.pki import AUTOCHAIN_PREFIX
                    if name.startswith(AUTOCHAIN_PREFIX):
                        exists = name in (get_config_ca_certificate() or {})
                if not exists:
                    raise dozenos.opmode.UnconfiguredSubsystem(unconf_message)
...
def get_config_ca_certificate(name=None):
    ...
    for cert_name, cert_conf in (get_config_certificate() or {}).items():
        ...
        for autochain_name, chain_entry in acme_chain_ca_entry(
                dozenos_certbot_dir, cert_name).items():
...
def generate_certificate_request(
    ...
    subject['common_name'] = ask_input('Enter common name:', default='dozenos.io')
    subject_alt_names = None

    if ask_san and ask_yes_no('Do you want to configure Subject Alternative Names?'):
        print(
            'Enter alternative names in a comma separate list, example: ipv4:1.1.1.1,ipv6:fe80::1,dns:dozenos.net,rfc822:user@dozenos.local'
        )
...
def import_ca_certificate(
    ...
    if path:
        ...
        if url.scheme in ('', 'file'):
            ...
            cert_data = dozenos.remote.get_remote_config(path)
...
def generate_pki(
    ...
    try:
        if pki_type == 'ca':
            ...
            # requires the dozenos_libexec_dir environment variable to be set
            os.environ['dozenos_libexec_dir'] = '/usr/libexec/dozenos'
...
def renew_certbot(raw: bool, force: typing.Optional[bool] = False):
    ...
    dozenos_conf_scripts_dir = directories['conf_mode']

...
def _get_raw_data_sas():
    try:
        get_sas = dozenos.ipsec.get_vici_sas()
        sas = convert_data(get_sas)
        return sas
    except dozenos.ipsec.ViciInitiateError as err:
        raise dozenos.opmode.UnconfiguredSubsystem(err)
...
def _get_convert_data_connections():
    try:
        get_connections = dozenos.ipsec.get_vici_connections()
...
def _initiate_peer_with_childsas(
    ...
    try:
        con_list: list = dozenos.ipsec.get_vici_connection_by_name(peer)
        if not con_list:
            raise dozenos.opmode.IncorrectValue(
                f"Peer's {peer} SA(s) not loaded. Initiation was failed"
            )
        childsa_name_list: list = _get_con_childsa_name_list(con_list)

        if not tunnel_sw:
            dozenos.ipsec.vici_initiate_all_child_sa_by_ike(peer, childsa_name_list)
            print(f'Peer {peer} initiate result: success')
            return

        if tunnel_sw in childsa_name_list:
            dozenos.ipsec.vici_initiate_all_child_sa_by_ike(peer, [tunnel_sw])
            print(f'Peer {peer} tunnel {tunnel} initiate result: success')
            return

        raise dozenos.opmode.IncorrectValue(f'Peer {peer} SA {tunnel} not found, aborting')

    except dozenos.ipsec.ViciInitiateError as err:
        raise dozenos.opmode.UnconfiguredSubsystem(err)
    except dozenos.ipsec.ViciCommandError as err:
        raise dozenos.opmode.IncorrectValue(err)
...
def _terminate_peer(peer: str, tunnel: typing.Optional[str] = None) -> None:
    ...
    tunnel_sw = _get_tunnel_sw_format(peer, tunnel)
    try:
        sa_list: list = dozenos.ipsec.get_vici_sas_by_name(peer, tunnel_sw)
        if sa_list:
            if tunnel:
                childsa_id_list: list = _get_childsa_id_list(sa_list)
                if childsa_id_list:
                    dozenos.ipsec.terminate_vici_by_name(peer, tunnel_sw)
...
def reset_all_peers() -> None:
    ...
    if sitetosite_list:
        for peer_name in sitetosite_list:
            ...
            except dozenos.opmode.IncorrectValue as err:
                print(err)
        print('Peers reset result: success')
    else:
        raise dozenos.opmode.UnconfiguredSubsystem(
...
def reset_ra(username: typing.Optional[str] = None):
    ...
    if list_sa_id:
        dozenos.ipsec.terminate_vici_ikeid_list(list_sa_id)
...
def reset_profile_dst(profile: str, tunnel: str, nbma_dst: str):
    if profile and tunnel and nbma_dst:
        ...
        try:
            # Get IKE SAs
            sa_list = convert_data(dozenos.ipsec.get_vici_sas_by_name(ike_sa_name, None))
            if not sa_list:
                raise dozenos.opmode.IncorrectValue(
            ...
            # terminate IKE SAs
            dozenos.ipsec.terminate_vici_ikeid_list(
                list(
                    [
    ...
    if profile and tunnel:
        ...
        try:
            ...
            sa_list: list = convert_data(
                dozenos.ipsec.get_vici_sas_by_name(ike_sa_name, None)
            )
            if not sa_list:
                raise dozenos.opmode.IncorrectValue(
                    f'SA(s) for profile {profile} tunnel {tunnel} not found, aborting'
            ...
            # terminate IKE SAs
            dozenos.ipsec.terminate_vici_by_name(ike_sa_name, None)
...
def show_ra_detail(
    ...
    if not list_sa:
        raise dozenos.opmode.IncorrectValue('No active connections found, aborting')
    if raw:
        return list_sa

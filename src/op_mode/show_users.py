...
filters = {
    ...
    'dozenos': lambda user: user.user_type == 'dozenos',
    'other': lambda user: user.user_type != 'dozenos',
...
def list_users():
    cfg = Config()
    dozenos_users = cfg.list_effective_nodes('system login user')
    users = []
    with open('/var/log/lastlog', 'rb') as lastlog_file:
        for entry in get_local_passwd_entries():
            ...
            user_info = UserInfo(
                uid, name,
                user_type='dozenos' if name in dozenos_users else 'other',
...
def main():
    ...
    parser.add_argument('type', nargs='?', choices=['all', 'dozenos', 'other', 'locked'])

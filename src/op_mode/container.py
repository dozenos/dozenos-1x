...
def clean_layer(name: str) -> int:
    ...
    unit = f"dozenos-container-{name}.service"
...
def add_image(name: str):
    ...
    is defined within DozenOS CLI, credentials are used to login before pull """
        ...
        for registry, registry_config in container['registry'].items():
            ...
            if 'authentication' in registry_config:
                ...
                if {'username', 'password'} <= set(registry_config['authentication']):
                    ...
                    rc, out = rc_cmd(cmd)
                    if rc != 0: raise dozenos.opmode.InternalError(out)

    rc, output = rc_cmd(f'podman image pull {name}', buffered=False)
    if rc != 0:
        raise dozenos.opmode.InternalError(output)
...
def delete_image(name: str, force: typing.Optional[bool] = False):
    ...
    for image in name:
        ...
        if rc != 0:
            raise dozenos.opmode.InternalError(ancestor)
...
def restart(name: str):
    ...
    rc, output = rc_cmd(f'systemctl restart dozenos-container-{name}.service')
    if rc != 0:
...
def show_log(name: str, follow: bool = False, raw: bool = False):
    ...
    if log_type == 'k8s-file':
        ...
        if follow:
            log_command_list = ['journalctl', '--follow', '--unit', f'dozenos-container-{name}.service']
        else:
            log_command_list = ['journalctl', '-e', '--no-pager', '--unit', f'dozenos-container-{name}.service']
    elif log_type == 'none':
        print(f'Container "{name}" has disabled logs.')
        return None
    else:
        raise dozenos.opmode.InternalError(f'Unknown log type "{log_type}" for container "{name}".')
    ...
    try:
        ...
        raise dozenos.opmode.InternalError(f"Error starting logging command: {e} ")
    return None

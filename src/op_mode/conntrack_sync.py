...
from dozenos.configquery import CliShellApiConfigQuery
...
def is_configured():
    ...
    if not config.exists(['service', 'conntrack-sync']):
        raise dozenos.opmode.UnconfiguredSubsystem("conntrack-sync is not configured!")

def send_bulk_update():
    """ send bulk update of internal-cache to other systems """
    tmp = run(f'{conntrackd_bin} -C {conntrackd_config} -B')
    if tmp > 0:
        raise dozenos.opmode.Error('Failed to send bulk update to other conntrack-sync systems')

def request_sync():
    """ request resynchronization with other systems """
    tmp = run(f'{conntrackd_bin} -C {conntrackd_config} -n')
    if tmp > 0:
        raise dozenos.opmode.Error('Failed to request resynchronization of external cache')
...
def flush_cache(direction):
    ...
    if tmp > 0:
        raise dozenos.opmode.Error('Failed to clear {direction} cache')
...
def show_statistics(raw: bool):
    if raw:
        raise dozenos.opmode.UnsupportedOperation("Machine-readable conntrack-sync statistics are not available yet")
    else:
        is_configured()

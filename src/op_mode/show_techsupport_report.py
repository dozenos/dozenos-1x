...
from dozenos.utils.process import wrap_op as op
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(description='DozenOS tech-support command collector')
    p.add_argument(
        '--outdir',
        type=Path,
        default=None,
    ...
    p.add_argument(
        ...
        help=(
            ...
            'Example: --reports dozenos-main-info'
        ),
    )
    p.add_argument(
...
def show_physical_interface_statistics(r: Runner):
    ...
    for path in Path('/run/udev/dozenos').glob('*'):
        if path.is_file():
...
REPORTS: dict[str, tuple[BaseSpec]] = {
    'dozenos-main-info': (
        CommandSpec('DozenOS version and package info', op('show version')),
        CommandSpec(
        ...
        CommandSpec(
            ...
            'cat /config/scripts/dozenos-preconfig-bootup.script',
            report_only=True,
        ),
        CommandSpec(
            'User startup script (postconfig)',
            'cat /config/scripts/dozenos-postconfig-bootup.script',
            report_only=True,
        ),
    ),
    'routing-info': (
    ...
    'nftables-info': (
        ...
        CommandSpec('DozenOS firewall configuration', op('show firewall')),
        CommandSpec('DozenOS firewall zone policy', op('show firewall zone-policy')),
    ),

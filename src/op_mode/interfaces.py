...
from dozenos.ifconfig import VRRP
...
def _show_raw(data: list, intf_name: str):
    if intf_name is not None and len(data) <= 1:
        try:
            ...
            raise dozenos.opmode.UnconfiguredObject(
...
def show_vlan_to_vni(raw: bool, intf_name: typing.Optional[str],
    ...
    if not interface_exists(intf_name):
        raise dozenos.opmode.UnconfiguredObject(f"Interface {intf_name} does not exist\n")
    ...
    if not tunnel_data:
        if vid == "all":
            raise dozenos.opmode.UnconfiguredObject(f"No VLAN-to-VNI mapping found for interface {intf_name}\n")
        else:

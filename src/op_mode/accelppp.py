...
def _get_config_settings(protocol):
    '''Get config dict from DozenOS configuration'''
...
def _get_raw_statistics(accel_output, pattern, protocol):
    return {
        **dozenos.accel_ppp.get_server_statistics(accel_output, pattern, sep=':'),
...
def _get_raw_sessions(port):
    ...
    output = dozenos.accel_ppp.accel_cmd(port, cmd_options)
    parsed_data: list[dict[str, str]] = dozenos.accel_ppp.accel_out_parse(
        output.splitlines())
    return parsed_data
...
def show_sessions(raw: bool, protocol: str):
    ...
    return dozenos.accel_ppp.accel_cmd(port,

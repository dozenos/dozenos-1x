...
from dozenos.configdiff import Diff, get_config_diff
...
from dozenos.firewall import fqdn_config_parse
from dozenos.geoip import geoip_refresh, geoip_update
...
domain_resolver_usage = '/run/use-dozenos-domain-resolver-firewall'
firewall_config_dir = "/config/firewall"

sysctl_file = r'/run/sysctl/10-dozenos-firewall.conf'
...
def apply(firewall):
    ...
    if dict_search_args(firewall, 'group', 'remote_group') or dict_search_args(firewall, 'group', 'domain_group') or firewall['ip_fqdn'].items() or firewall['ip6_fqdn'].items():
        text = f'# Automatically generated by firewall.py\nThis file indicates that dozenos-domain-resolver service is used by the firewall.\n'
        write_file(domain_resolver_usage, text)
    else:
        if os.path.exists(domain_resolver_usage):
            os.unlink(domain_resolver_usage)
        if not glob('/run/use-dozenos-domain-resolver*'):
            domain_action = 'stop'
    call(f'systemctl {domain_action} dozenos-domain-resolver.service')

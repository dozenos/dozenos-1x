...
from dozenos.ifconfig import WireGuardIf
...
from dozenos.utils.network import is_wireguard_key_pair
...
def apply(wireguard):
    ...
    domain_resolver_usage = '/run/use-dozenos-domain-resolver-interfaces-wireguard-' + wireguard['ifname']
    ...
    if 'peers_need_resolve' in wireguard and len(wireguard['peers_need_resolve']) > 0 and 'disable' not in wireguard:
        ...
        text = f'# Automatically generated by interfaces_wireguard.py\nThis file indicates that dozenos-domain-resolver service is used by the interfaces_wireguard.\n'

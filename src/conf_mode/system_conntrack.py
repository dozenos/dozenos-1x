...
sysctl_file = r'/run/sysctl/10-dozenos-conntrack.conf'
nftables_ct_file = r'/run/nftables-ct.conf'
dozenos_conntrack_logger_config = r'/run/dozenos-conntrack-logger.conf'
...
def generate(conntrack):
    ...
    if 'log' not in conntrack:
        ...
        if os.path.exists(dozenos_conntrack_logger_config):
            os.unlink(dozenos_conntrack_logger_config)
    ...
    if 'log' in conntrack:
        ...
        write_file(dozenos_conntrack_logger_config, log_conf_json)
...
def apply(conntrack):
    ...
    if 'log' in conntrack:
        ...
        call(f'systemctl stop dozenos-conntrack-logger.service')

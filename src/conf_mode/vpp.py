...
from dozenos.config import Config, config_dict_merge
from dozenos.configdep import set_dependents
from dozenos.configdep import call_dependents
from dozenos.configdict import node_changed, is_member
from dozenos.configverify import verify_interface_exists
from dozenos.configverify import verify_virtual_interface_exists
from dozenos.ifconfig import Section
from dozenos.logger import getLogger
...
from dozenos.utils.kernel import list_loaded_modules
from dozenos.utils.process import call
from dozenos.utils.process import is_systemd_service_active

from dozenos.vpp import VPPControl
from dozenos.vpp import control_host
from dozenos.vpp import VppNotRunningError
from dozenos.vpp.config_verify import (
...
from dozenos.vpp.config_resource_checks import memory
from dozenos.vpp.config_filter import iface_filter_eth
from dozenos.vpp.utils import EthtoolGDrvinfo
from dozenos.vpp.utils import cli_ethernet_with_vifs_ifaces
from dozenos.vpp.configdb import JSONStorage
...
def verify(config):
    ...
    for iface, iface_config in config['settings']['interface'].items():
        ...
        if not _is_device_allowed(config, iface):
            raise ConfigError(
                f'NIC used by "{iface}" is not validated for VPP on DozenOS. '

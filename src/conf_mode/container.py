...
from dozenos.template import bracketize_ipv6
...
def verify(container):
    ...
    if 'name' in container:
        ...
        for name, container_config in container['name'].items():
            ...
            # shared container image storage across all DozenOS images. A user can
            # delete a container image from the system, boot into another version
            # of DozenOS and then it would fail to boot. This is to prevent any
...
def generate_quadlet_options(name, container_config, host_ident, network_config):
    ...
    if 'network' in container_config:
        for network in container_config['network']:
            ...
            out.append(f'Network=dozenos-{network}.network{opts_str}')
...
def generate(container):
    ...
    if 'network' in container:
        for network, network_config in container['network'].items():
            file_path = os.path.join(quadlet_unit_path, f'dozenos-{network}.network')
    ...
    if 'name' in container:
        ...
        for name, container_config in container['name'].items():
            ...
            file_path = os.path.join(quadlet_unit_path, f'dozenos-container-{name}.container')
...
def apply(container):
    ...
    if 'container_remove' in container:
        quadlet_paths = [f'dozenos-container-{name}.container' for name in container['container_remove']]
        run(['podman', 'quadlet', 'rm', '-f'] + quadlet_paths)

    # Delete old networks if needed
    if 'network_remove' in container:
        quadlet_paths = [f'dozenos-{name}.network' for name in container['network_remove']]
        run(['podman', 'quadlet', 'rm', '-f'] + quadlet_paths)
    ...
    if 'name' in container:
        for name, container_config in container['name'].items():
            ...
            if 'disable' in container_config:
                ...
                if name in tmp:
                    run(['podman', 'quadlet', 'rm', '-f', f'dozenos-container-{name}.container'])
                continue

            if 'container_restart' in container and name in container['container_restart']:
                cmdl(['systemctl', 'restart', f'dozenos-container-{name}'])

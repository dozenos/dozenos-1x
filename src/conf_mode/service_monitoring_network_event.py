...
dozenos_network_event_logger_config = r'/run/dozenos-network-event-logger.conf'
...
def generate(monitoring):
    if not monitoring:
        # Delete config
        if os.path.exists(dozenos_network_event_logger_config):
            os.unlink(dozenos_network_event_logger_config)

        return None

    # Create config
    log_conf_json = json.dumps(monitoring, indent=4)
    write_file(dozenos_network_event_logger_config, log_conf_json)
...
def apply(monitoring):
    ...
    systemd_service = 'dozenos-network-event-logger.service'

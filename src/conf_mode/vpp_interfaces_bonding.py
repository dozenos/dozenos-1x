...
from dozenos.ifconfig.vpp import VPPBondInterface
from dozenos.vpp.config_deps import deps_bond_dict
from dozenos.vpp.config_deps import deps_bridge_dict
from dozenos.vpp.config_deps import deps_xconnect_dict
from dozenos.vpp.config_verify import verify_member_conflicts
...
from dozenos.vpp.config_verify import verify_vpp_interface_not_in_feature
from dozenos.vpp.utils import cli_ifaces_list


def _get_bond_mode(mode_name: str) -> int:
    """Convert DozenOS CLI name bonding mode to VPP compatible"""
    mode_mapping = {
        'round-robin': 1,
        'active-backup': 2,
        'xor-hash': 3,
...
def _get_bond_lb(lb_name: str) -> int:
    """Convert DozenOS CLI name bonding load balance to VPP compatible"""
    lb_mapping = {
        'layer2': 0,
        'layer2+3': 2,
        'layer3+4': 1,
...
def get_config(config=None) -> dict:
    ...
    Args:
        config (dozenos.config.Config, optional): The DozenOS configuration dictionary
    Returns:

...
from dozenos.base import DeprecationWarning
...
from dozenos.defaults import SSH_DSA_DEPRECATION_WARNING
...
from dozenos.utils.auth import MIN_USER_UID
...
from dozenos.utils.file import move_recursive
...
limits_file = "/etc/security/limits.d/10-dozenos.conf"
...
login_motd_dsa_warning = r'/run/motd.d/92-dozenos-user-dsa-deprecation-warning'
...
def verify(login):
    if 'rm_users' in login:
        # This check is required as the script is also executed from dozenos-router
    ...
    if 'user' in login:
        ...
        for user, user_config in login['user'].items():
            ...
            # DozenOS CLI user which already exists as system user
...
def generate(login):
    ...
    # /etc/security/limits.d/10-dozenos.conf
    ...
    with open('/etc/dozenos/operators.json', 'w') as of:

...
from dozenos.utils.network import is_loopback_addr
...
dozenos_config_file = "/etc/lldpd.d/01-dozenos.conf"
base = ['service', 'lldp']
...
def generate(lldp):
    ...
    render(dozenos_config_file, 'lldp/dozenos.conf.j2', lldp)
...
def apply(lldp):
    ...
    if lldp:
        ...
        if os.path.isfile(dozenos_config_file):
            os.unlink(dozenos_config_file)

...
def generate(dyndns):
    ...
    for name in dyndns['name']:
        ...
        # - 'password' in ddclient.conf instead of 'key' in dozenos conf
        if dyndns['name'][name]['protocol'] == 'nsupdate':
            dyndns['name'][name]['password'] = dyndns['name'][name].pop('key')

        # porkbun uses:
        # - 'root-domain' in ddclient.conf instead of 'zone' in dozenos conf
        # - 'apikey' in ddclient.conf instead of 'username' in dozenos conf
        # - 'secretapikey' in ddclient.conf instead of 'password' in dozenos conf

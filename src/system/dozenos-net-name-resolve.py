...
#
# The old udev-time approach (src/udev/dozenos_net_name, invoked straight from
...
dozenos_udev_dir = directories['dozenos_udev_dir']
status_file = Path('/run/dozenos-net-name-resolve.json')
...
def compute_bootstrap_plan(configured: dict, current: dict, existing_plan: dict) -> dict:
    ...
    config.boot by dozenos-interface-rescan.py the same way a real hw-id
    match would.
...
def sync_rescan_hints(current_state: dict, configured: dict,
    ...
    """Leave a hint under dozenos_udev_dir for every interface that has no
    hw-id configured yet, so dozenos-interface-rescan.py can auto-populate
    config.boot. Remove stale/no-longer-hw-id-needed hints.
    """
    try:
        Path(dozenos_udev_dir).mkdir(parents=True, exist_ok=True)
    except OSError as e:
        logger.critical(f'error creating rescan hint directory: {e}')
        return

    for name, mac in current_state.items():
        hint = Path(dozenos_udev_dir) / name
    ...
    for old_name in stale_names:
        if old_name not in current_state:
            (Path(dozenos_udev_dir) / old_name).unlink(missing_ok=True)
...
def main():
    ...
    if all_pending or any(mac not in configured for mac in current.values()):
        ...
        # udev-time name - this is what dozenos-interface-rescan.py will
    ...
    # that node - so its rescan hint lands there and dozenos-interface-
    ...
    # dozenos-interface-rescan.py write the hw-id into the existing node.

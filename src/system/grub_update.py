...
from dozenos.system import disk, grub, image, compat, SYSTEM_CFG_VER
...
if __name__ == '__main__':
    ...
    dozenos_menuentries = compat.parse_menuentries(grub_cfg_main)
    dozenos_versions = compat.find_versions(dozenos_menuentries)
    ...
    # find default values
    default_entry = dozenos_menuentries[int(vars['default'])]
    default_settings = {
    ...
    grub_cfg_platform = f'{root_dir}/{grub.CFG_DOZENOS_PLATFORM}'
    grub_cfg_menu = f'{root_dir}/{grub.CFG_DOZENOS_MENU}'
    grub_cfg_options = f'{root_dir}/{grub.CFG_DOZENOS_OPTIONS}'

    Path(image.GRUB_DIR_DOZENOS).mkdir(exist_ok=True)
    ...
    # create menu entries
    for dozenos_ver in dozenos_versions:
        boot_opts = None
        for entry in dozenos_menuentries:
            if entry.get('version') == dozenos_ver and entry.get(
                    'bootmode') == 'normal':
                boot_opts = entry.get('boot_opts')
        grub.version_add(dozenos_ver, root_dir, boot_opts)
    ...
    grub.sort_inodes(f'{root_dir}/{grub.GRUB_DIR_DOZENOS}')
    grub.sort_inodes(f'{root_dir}/{grub.GRUB_DIR_DOZENOS_VERS}')

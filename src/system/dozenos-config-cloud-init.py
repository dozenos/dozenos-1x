...
config_file = '/etc/cloud/cloud.cfg.d/20_dozenos_network.cfg'
...
def need_to_check() -> bool:
    ...
    ci_config_dozenos = Path('/etc/cloud/cloud.cfg.d/20_dozenos_custom.cfg')
    if not ci_config_dozenos.exists():
        logger.info(
    ...
    try:
        config = safe_load(ci_config_dozenos.read_text())
    except:
        logger.error('Cloud-init config file has a wrong format')
        return False

    # check if we have in config configured option
    # dozenos_config_options:
    #   network_preconfigure: true
    if not config.get('dozenos_config_options', {}).get('network_preconfigure'):
        logger.info(

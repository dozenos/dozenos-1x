...
# ordering corner case (dozenos-net-name-resolve must run from inside
# dozenos-router, after the config directory is mounted/decrypted, not as an
...
resolver = prepare_module(
    os.path.join(_here, '../system/dozenos-net-name-resolve.py'),
    'dozenos_net_name_resolve')
dozenos_net_name = prepare_module(
    os.path.join(_here, '../udev/dozenos_net_name'),
    'dozenos_net_name')
...
def setUpModule():
    ...
    # are routinely run from one (the DozenOS build container). Pin the detection
...
class TestFindAvailableCrashFix(unittest.TestCase):
    ...
    def test_dozenos_net_name_empty_dict(self):
        self.assertEqual(dozenos_net_name.find_available({}, 'eth'), 'eth0')

    def test_dozenos_net_name_no_digit_suffix(self):
        self.assertEqual(
            dozenos_net_name.find_available({'m': 'wlan3'}, 'eth'), 'eth0')

    def test_dozenos_net_name_contiguous(self):
        intfs = {'a': 'eth0', 'b': 'eth1'}
        self.assertEqual(dozenos_net_name.find_available(intfs, 'eth'), 'eth2')

    def test_dozenos_net_name_is_available(self):
        self.assertTrue(dozenos_net_name.is_available({'a': 'eth0'}, 'eth1'))
        self.assertFalse(dozenos_net_name.is_available({'a': 'eth0'}, 'eth0'))

    def test_dozenos_net_name_mod_ifname(self):
        self.assertEqual(dozenos_net_name.mod_ifname('e5'), 'eth3')
        self.assertEqual(dozenos_net_name.mod_ifname('e2'), 'eth0')
        self.assertEqual(dozenos_net_name.mod_ifname('e1'), 'eth1')
        self.assertEqual(dozenos_net_name.mod_ifname('wlan0'), 'wlan0')
...
class TestSyncRescanHints(unittest.TestCase):
    ...
    dozenos-interface-rescan.py, already-configured interfaces must not, and
    ...
    def setUp(self):
        self.tmp = tempfile.mkdtemp()
        self.addCleanup(shutil.rmtree, self.tmp, ignore_errors=True)
        self._orig_dir = resolver.dozenos_udev_dir
        resolver.dozenos_udev_dir = self.tmp
        self.addCleanup(setattr, resolver, 'dozenos_udev_dir', self._orig_dir)
...
class TestMainFirstBootBootstrap(unittest.TestCase):
    ...
    freeze that into the rescan hints dozenos-interface-rescan.py persists.
    """

    def setUp(self):
        self.udev_dir = tempfile.mkdtemp()
        self.addCleanup(shutil.rmtree, self.udev_dir, ignore_errors=True)
        self._orig_udev_dir = resolver.dozenos_udev_dir
        resolver.dozenos_udev_dir = self.udev_dir
        self.addCleanup(setattr, resolver, 'dozenos_udev_dir', self._orig_udev_dir)
    ...
    def test_reported_reproduction_end_to_end(self):
        ...
        # 'eth1' so dozenos-interface-rescan.py can refill its hw-id.
        configured = {'m0': 'eth0', 'm2': 'eth2'}
        pending = {'ethernet': {'eth1'}, 'wireless': set()}
        ...
        # dozenos-interface-rescan.py can write the real hw-id into the
    ...
    def test_gap_backfill_and_pending_node_coexisting_stays_ambiguous(self):
        # the exact dozenos-build check-qemu-install --ifnametest shape: one
...
class TestMainInContainer(unittest.TestCase):
    ...
    def test_naming_pass_is_skipped_entirely(self):
        ...
        # no status file means dozenos-router's warn_missing_interface_hardware()
...
class TestWriteStatus(unittest.TestCase):
    ...
    reported so dozenos-router can surface a boot-time warning - it must
...
class TestBootOrdering(unittest.TestCase):
    ...
    /opt/vyatta/etc/config/config.boot does not exist until dozenos-router
    mounts (and, for an encrypted config volume, decrypts) the config
    directory itself. dozenos-net-name-resolve must be started explicitly
    from inside dozenos-router after that point, and must not be
    reintroduced as an independently-scheduled unit ordered before
    dozenos-router/network-pre.target/cloud-init.
    """

    def setUp(self):
        path = os.path.join(_here, '../init/dozenos-router')
        with open(path) as f:
    ...
    def _first_index(self, needle):
        for i, line in enumerate(self.lines):
            if needle in line:
                return i
        self.fail(f"'{needle}' not found in src/init/dozenos-router")
    ...
    def test_resolver_runs_after_migrate_and_before_config_apply(self):
        ...
        resolve = self._first_index(
            'systemctl start dozenos-net-name-resolve.service')
    ...
    def test_service_unit_not_independently_ordered_before_dozenos_router(self):
        path = os.path.join(_here, '../systemd/dozenos-net-name-resolve.service')
        with open(path) as f:
            unit = f.read()
        self.assertNotIn('Before=dozenos-router.service', unit)

...
import dozenos.utils.network
from unittest import TestCase

class TestDozenOSUtilsNetwork(TestCase):
    def setUp(self):
        pass

    def test_is_addr_assigned(self):
        self.assertTrue(dozenos.utils.network.is_addr_assigned('127.0.0.1'))
        self.assertTrue(dozenos.utils.network.is_addr_assigned('::1'))
        self.assertFalse(dozenos.utils.network.is_addr_assigned('127.251.255.123'))

    def test_is_ipv6_link_local(self):
        self.assertFalse(dozenos.utils.network.is_ipv6_link_local('169.254.0.1'))
        self.assertTrue(dozenos.utils.network.is_ipv6_link_local('fe80::'))
        self.assertTrue(dozenos.utils.network.is_ipv6_link_local('fe80::affe:1'))
        self.assertTrue(dozenos.utils.network.is_ipv6_link_local('fe80::affe:1%eth0'))
        self.assertFalse(dozenos.utils.network.is_ipv6_link_local('2001:db8::'))
        self.assertFalse(dozenos.utils.network.is_ipv6_link_local('2001:db8::%eth0'))
        self.assertFalse(dozenos.utils.network.is_ipv6_link_local('DozenOS'))
    ...
    def test_is_loopback_addr(self):
        ...
        self.assertTrue(dozenos.utils.network.is_loopback_addr('::1'))
    ...
    def test_check_port_availability(self):
        ...
        self.assertTrue(dozenos.utils.network.check_port_availability('127.0.0.1', 8080))
        self.assertTrue(dozenos.utils.network.check_port_availability(None, 8080, protocol='udp'))
        # We do not have 192.0.2.1 configured on this system
        self.assertFalse(dozenos.utils.network.check_port_availability('192.0.2.1', 443))
        # We do not have 2001:db8::1 configured on this system
        self.assertFalse(dozenos.utils.network.check_port_availability('2001:db8::1', 80, protocol='udp'))

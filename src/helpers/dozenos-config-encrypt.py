...
dm_device = '/dev/mapper/dozenos_config'
...
def encrypt_config(key, recovery_key=None, is_tpm=True):
    ...
    try:
        ...
        if os.path.exists('/dev/mapper/dozenos_config'):
            run('cryptsetup -q close dozenos_config')
...
if __name__ == '__main__':
    ...
    if args.enable and not tpm_exists:
        print('WARNING: DozenOS will boot into a default config when encrypted without a TPM')

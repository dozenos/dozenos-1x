...
from dozenos.defaults import config_status
...
def failsafe(config_file_name):
    ...
    if not config.exists(['system', 'login', 'user', 'dozenos',
                          'authentication', 'encrypted-password']):
        print("No password entry in default config file;")
        print("unable to recover password for user 'dozenos'.")
        sys.exit(1)
    else:
        passwd = config.return_value(['system', 'login', 'user', 'dozenos',
                                      'authentication',
                                      'encrypted-password'])
...
if __name__ == '__main__':
    ...
    session = ConfigSession(os.getpid(), 'dozenos-boot-config-loader')

...
from dozenos.utils.permission import get_cfg_group_id

debug = False

dozenos_udev_dir = directories['dozenos_udev_dir']
dozenos_log_dir = directories['log']
log_file = os.path.splitext(os.path.basename(__file__))[0]
dozenos_log_file = os.path.join(dozenos_log_dir, log_file)

logger = logging.getLogger(__name__)
handler = logging.FileHandler(dozenos_log_file, mode='a')
...
def get_new_interfaces() -> dict:
    ...
    for intf in os.listdir(dozenos_udev_dir):
        path = os.path.join(dozenos_udev_dir, intf)
        try:
            with open(path) as f:
                hwid = f.read().rstrip()
        except OSError as e:
...
def main():
    ...
    # logging config file
    os.chown(dozenos_log_file, 0, get_cfg_group_id())
    os.chmod(dozenos_log_file,

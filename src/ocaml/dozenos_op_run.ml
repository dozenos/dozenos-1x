(*
 * dozenos-op-run: the wrapper for executing operational mode commands.
 *
...
(* Global constants *)
let op_def_file = "/usr/share/dozenos/op_cache.json"
let permissions_file = "/etc/dozenos/operators.json"
let dozenos_admin_group_name = "vyattacfg"
...
type options = {
  ...
  (* The original DozenOS command,
     like "show interfaces ethernet",
     for debugging and for substitutions of $@/$*
   *)
  dozenos_command: string;
}

let default_options = {
  dry_run = false;
  debug = false;
  dozenos_command = "<DozenOS command is undefined>";
}
...
let is_admin () =
  ...
  if Unix.getuid () = 0 then
    ...
    (* Otherwise, check if the user is a DozenOS admin *)
    let admin_group = Unix.getgrnam dozenos_admin_group_name in
    let user_groups = Unix.getgroups () in
    match (Array.find_opt ((=) admin_group.gr_gid) user_groups) with
    | Some _ ->
      let () = Logs.debug @@ fun m -> m "The user is a DozenOS admin, permission checks will be skipped" in
      true
    | None ->
      let () = Logs.debug @@ fun m -> m "The user does not have DozenOS admin permissions" in
      false
  end
...
let has_unsafe_characters cmd =
  (* XXX: this function is highly restrictive now,
     ...
     cannot happen down the line inside DozenOS op mode scripts.
...
let check_command_permissions perms cmd =
  ...
  (* DozenOS admins can execute any commands without restrictions *)
...
let render_command opts env command_tmpl =
  ...
  let dozenos_command = opts.dozenos_command in
  Pcre2.replace ~pat:{|\$[@*]|} ~templ:dozenos_command command
...
let run_external_command opts env command_tmpl =
  ...
  let env = [|
    (* A knowingly safe executable lookup path.
       ...
       Executables in DozenOS-specific directories are referred to by absolute paths
    ...
    (* DozenOS-specific variables *)
    make_var "dozenos_data_dir" "/usr/share/dozenos";
    make_var "dozenos_validators_dir" "/usr/libexec/dozenos/validators";
    make_var "dozenos_completion_dir" "/usr/libexec/dozenos/completion";
    make_var "dozenos_libexec_dir" "/usr/libexec/dozenos";
    make_var "dozenos_op_scripts_dir" "/usr/libexec/dozenos/op_mode";
  |]
  in
  let shell = "/bin/sh" in
  (* We use execve with an absolute path to Bourne shell rather than execvpe
     so that a user trying to do PATH=/bad/place dozenos-op-run
...
(* Command lookup *)
let rec run_dozenos_command opts ?(env=[]) ?(parent="") node cmd_words =
  ...
  | w :: ws ->
    ...
    | Some child_node ->
      ...
      run_dozenos_command opts ~env:env ~parent:w child_node ws
    | None ->
      ...
      | "tagNode", None ->
        ...
        | _ as ws ->
          run_dozenos_command opts ~env:env ~parent:w node ws
        end
      | "node", Some vtn ->
        ...
        | _ ->
           ...
           *)
          run_dozenos_command opts ~env:env ~parent:parent vtn ws
        end
...
%s is the DozenOS operational command wrapper.
...
let get_args () =
  ...
  ({!opts with dozenos_command=(String.concat " " args)}, args)

let () =
  let debug =
    (* For simplicity, we check for the existence
       of the DOZENOS_DEBUG environment variable,
       rather than for specific values.
     *)
    match Unix.getenv "DOZENOS_DEBUG" with
    | _ -> true
  ...
  let () = Logs.debug @@ fun m -> m "Executing DozenOS command [%s]" options.dozenos_command in
  try
    check_command_permissions permissions args;
    Unix.setuid 0;
    run_dozenos_command options ~env:[] ~parent:"" op_defs args
  with
  | Permission_error ->
    Printf.fprintf stderr "You do not have a permission to execute DozenOS command [%s]\n"
      options.dozenos_command;
    exit 1
  | Invalid_command msg ->
    Printf.fprintf stderr "Invalid command [%s]: %s\n" options.dozenos_command msg;
    exit 1
  | Command_error msg ->

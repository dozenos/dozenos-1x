...
def get_nft_filter_chains():
    ...
    """
    try:
        nft = cmd('/usr/sbin/nft --json list table ip dozenos_filter')
    except Exception:
        return []
    nft = json.loads(nft)
    chain_list = []
...
def get_nftables_details(name):
    ...
    """
    command = f'/usr/sbin/nft list chain ip dozenos_filter {name}'
    try:
        results = cmd(command)
    except:
        return {}
...
def get_nft_telegraf(name):
    ...
    for rule, rule_config in get_nftables_details(name).items():
        print(f'nftables,table=dozenos_filter,chain={name},'
              f'ruleid={rule} '

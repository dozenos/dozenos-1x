...
# configsession -- the write API for the DozenOS running config
...
from dozenos.utils.dict import dict_to_paths
...
from dozenos.base import Warning as Warn
from dozenos.defaults import DEFAULT_COMMIT_CONFIRM_MINUTES
from dozenos.configtree import ConfigTree
from dozenos.configtree import ConfigTreeError
from dozenos.configtree import delete_dict_from_masks
from dozenos.derivedtree import subtree_from_list_of_partial_paths
...
MIGRATE_LOAD_CONFIG = ['/usr/libexec/dozenos/dozenos-load-config.py']
MERGE_CONFIG = ['/usr/libexec/dozenos/dozenos-merge-config.py']
SAVE_CONFIG = ['/usr/libexec/dozenos/dozenos-save-config.py']
INSTALL_IMAGE = [
    '/usr/libexec/dozenos/op_mode/image_installer.py',
    '--action',
    'add',
    '--no-prompt',
    '--image-path',
...
IMPORT_PKI_NO_PROMPT = [
    '/usr/libexec/dozenos/op_mode/pki.py',
    'import_pki',
    '--no-prompt',
]
REMOVE_IMAGE = [
    '/usr/libexec/dozenos/op_mode/image_manager.py',
    '--action',
    'delete',
    '--no-prompt',
    '--image-name',
...
TRACEROUTE = [
    '/usr/libexec/dozenos/op_mode/mtr_execute.py',
    'mtr',
    '--for-api',
    '--report-mode',
    '--report-cycles',
...
def inject_dozenos_env(env):
    ...
    env['DOZENOS_HEADLESS_CLIENT'] = 'dozenos_http_api'
    env['vyatta_bindir'] = '/opt/vyatta/bin'
    env['vyatta_cfg_templates'] = '/opt/vyatta/share/vyatta-cfg/templates'
    env['vyatta_configdir'] = directories['dozenos_configdir']
    ...
    env['dozenos_bin_dir'] = '/usr/bin'
    env['dozenos_cfg_templates'] = '/opt/vyatta/share/vyatta-cfg/templates'
    env['dozenos_completion_dir'] = '/usr/libexec/dozenos/completion'
    env['dozenos_configdir'] = directories['dozenos_configdir']
    env['dozenos_conf_scripts_dir'] = '/usr/libexec/dozenos/conf_mode'
    env['dozenos_datadir'] = '/opt/vyatta/share'
    env['dozenos_datarootdir'] = '/opt/vyatta/share'
    env['dozenos_libdir'] = '/opt/vyatta/lib'
    env['dozenos_libexec_dir'] = '/usr/libexec/dozenos'
    env['dozenos_op_scripts_dir'] = '/usr/libexec/dozenos/op_mode'
    env['dozenos_op_templates'] = '/opt/vyatta/share/vyatta-op/templates'
    env['dozenos_prefix'] = '/opt/vyatta'
    env['dozenos_sbin_dir'] = '/usr/sbin'
    env['dozenos_validators_dir'] = '/usr/libexec/dozenos/validators'
    ...
    # if running the dozenos-configd daemon, inject the vyshim env var
    if is_systemd_service_running('dozenos-configd.service'):
...
class ConfigSession(object):
    ...
    def __init__(self, session_id, app=APP, shared=False):
        ...
        session_env = inject_dozenos_env(session_env)
        for k, v in env_list:
            session_env[k] = v
    ...
    def load_explicit(self, file_path):
        from dozenos.load_config import load
        from dozenos.load_config import LoadConfigError

...
// dozenos-config-version: "broadcast-relay@1:cluster@1:config-management@1:conntrack@3:conntrack-sync@2:dhcp-relay@2:dhcp-server@6:dhcpv6-server@1:dns-forwarding@3:firewall@5:https@2:interfaces@22:ipoe-server@1:ipsec@5:isis@1:l2tp@3:lldp@1:mdns@1:nat@5:ntp@1:pppoe-server@5:pptp@2:qos@1:quagga@8:rpki@1:salt@1:snmp@2:ssh@2:sstp@3:system@21:vrrp@2:dozenos-accel-ppp@2:wanloadbalance@3:webproxy@2:zone-policy@1"
...
from dozenos.xml_ref import component_version
...
REGEX_WARN_DOZENOS = r'(// Warning: Do not remove the following line.)'
REGEX_WARN_VYATTA = r'(/\* Warning: Do not remove the following line. \*/)'
REGEX_COMPONENT_VERSION_DOZENOS = r'// dozenos-config-version:\s+"([\w@:-]+)"\s*'
REGEX_COMPONENT_VERSION_VYATTA = (
    r'/\* === vyatta-config-version:\s+"([\w@:-]+)"\s+=== \*/'
)
REGEX_RELEASE_VERSION_DOZENOS = r'// Release version:\s+(\S*)\s*'
REGEX_RELEASE_VERSION_VYATTA = r'/\* Release version:\s+(\S*)\s*\*/'

CONFIG_FILE_VERSION = """\
// Warning: Do not remove the following line.
// dozenos-config-version: "{}"
// Release version: {}
"""

warn_filter_dozenos = re.compile(REGEX_WARN_DOZENOS)
...
regex_filter = {
    'dozenos': dict(
        zip(
            ['component', 'release'],
            [
                re.compile(REGEX_COMPONENT_VERSION_DOZENOS),
                re.compile(REGEX_RELEASE_VERSION_DOZENOS),
            ],
        )
    ),
    'vyatta': dict(
...
def version_info_from_file(config_file) -> VersionInfo:
    ...
    if len(parts := warn_filter_dozenos.split(config_str)) > 1:
        vintage = 'dozenos'
...
def version_info_from_system() -> VersionInfo:
    ...
    version_info = VersionInfo(component=sort_d, release=get_version(), vintage='dozenos')

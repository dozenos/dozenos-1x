# configtree -- a standalone DozenOS config file manipulation library (Python bindings)
...
# for type 'ConfigDict'
if TYPE_CHECKING:
    from dozenos.referencetree import ReferenceTree

BUILD_PATH = '/tmp/libdozenosconfig/_build/libdozenosconfig.so'
INSTALL_PATH = '/usr/lib/libdozenosconfig.so.0'
...
# libdozenosconfig is not safe for concurrent use: two threads inside it at once
...
# enter the library concurrently; dozenos-http-api does, running configure
...
# (error_message in libdozenosconfig/lib/bindings.ml), which each of the entry
...
class _Lib:
    """Lazily resolving, prototype-declaring wrapper around libdozenosconfig.
    ...
    def _raw(self, name):
        try:
            ...
            raise AttributeError(
                f'{name} is not a known libdozenosconfig entry point'
            ) from None
...
def get_lib(libpath=LIBPATH):
    """Load libdozenosconfig once per process.
...
class ConfigTree:
    ...
    def __init__(
        ...
        self.__migration = os.environ.get('DOZENOS_MIGRATION')
        if self.__migration:
            self.migration_log = logging.getLogger('dozenos.migrate')
    ...
    def set(self, path, value=None, replace=True):
        """Set new entry in DozenOS configuration.
...
def validate_tree_filter(
    config_tree,
    cache_path='/usr/share/dozenos/reftree.cache',
    validator_dir='/usr/libexec/dozenos/validators',
    libpath=LIBPATH,
):
    # pylint: disable=raise-missing-from
    try:

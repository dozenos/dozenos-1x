...
class ApiError(Exception):
    """Generic DozenOS HTTP API client error"""
...
class ApiClient:
    """Small helper for talking to DozenOS HTTP API using requests.

    Design goals:
    - minimal surface area (thin wrapper around requests)
    - consistent error handling + typed exceptions
    - safe defaults for DozenOS typical self-signed HTTPS usage (verify_tls=False)
    """

    _DEFAULT_HEADERS = {
        'Content-Type': 'application/json',
        'User-Agent': f'DozenOS/{get_version()}',
    }
    ...
    def post(
        ...
        *,
        ...
        # Most DozenOS endpoints in this repo expect 'key' inside body.
        body = dict(payload)

...
"""
A library for retrieving value dicts from DozenOS configs in a declarative fashion.
"""
import os
import json
...
def retrieve_config(path_hash, base_path, config):
    """
    Retrieves a DozenOS config as a dict according to a declarative description
    ...
    Args:
        ...
        config (dozenos.config.Config): A DozenOS config object
...
def get_removed_vlans(conf, path, dict):
    ...
    """
    from dozenos.configdiff import get_config_diff, Diff
...
def get_interface_dict(config, base, ifname='', recursive_defaults=True, with_pki=False):
    ...
    if not ifname:
        ...
        if 'DOZENOS_TAGNODE_VALUE' not in os.environ:
            raise ConfigError('Interface (DOZENOS_TAGNODE_VALUE) not specified')
        ifname = os.environ['DOZENOS_TAGNODE_VALUE']
...
def get_accel_dict(config, base, chap_secrets, with_pki=False):
    ...
    from dozenos.utils.cpu import get_half_cpus

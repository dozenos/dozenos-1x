...
from dozenos.configtree import diff_compare
...
from dozenos.version import get_full_version_data
...
commit_hooks = {
    'commit_revision': '01dozenos-commit-revision',
    'commit_archive': '02dozenos-commit-archive',
}
...
Proceed ?"""
        ...
        if self.reboot_unconfirmed:
            cmd = (
                f'sudo -b /usr/libexec/dozenos/commit-confirm-notify.py --reboot {minutes}'
            )
        else:
            cmd = f'sudo -b /usr/libexec/dozenos/commit-confirm-notify.py {minutes}'

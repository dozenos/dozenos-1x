...
from dozenos.utils.network import get_vxlan_vlan_tunnels
from dozenos.utils.network import get_vxlan_vni_filter
...
class VXLANIf(Interface):
    ...
    def _create(self):
        # This table represents a mapping from DozenOS internal config dict to
        ...
        for dozenos_key, iproute2_key in mapping.items():
            # dict_search will return an empty dict "{}" for valueless nodes like
            # "parameters.nolearning" - thus we need to test the nodes existence
            # by using isinstance()
            tmp = dict_search(dozenos_key, self.config)

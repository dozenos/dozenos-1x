...
class Interface(Control):
    ...
    def _cleanup_mss_rules(self, table, ifname):
        commands = []
        results = self._cmd(f'nft -a list chain {table} DOZENOS_TCP_MSS').split("\n")
        for line in results:
            if f'oifname "{ifname}"' in line:
                handle_search = re.search('handle (\d+)', line)
                if handle_search:
                    self._cmd(f'nft delete rule {table} DOZENOS_TCP_MSS handle {handle_search[1]}')
    ...
    def _cleanup_ipv4_source_validation_rules(self, ifname):
        results = self._cmd(f'nft -a list chain ip raw dozenos_rpfilter').split("\n")
        for line in results:
            if f'iifname "{ifname}"' in line:
                handle_search = re.search('handle (\d+)', line)
                if handle_search:
                    self._cmd(f'nft delete rule ip raw dozenos_rpfilter handle {handle_search[1]}')
    ...
    def set_ipv4_source_validation(self, mode):
        ...
        nft_prefix = f'nft insert rule ip raw dozenos_rpfilter iifname "{self.ifname}"'
    ...
    def _cleanup_ipv6_source_validation_rules(self, ifname):
        results = self._cmd(f'nft -a list chain ip6 raw dozenos_rpfilter').split("\n")
        for line in results:
            if f'iifname "{ifname}"' in line:
                handle_search = re.search('handle (\d+)', line)
                if handle_search:
                    self._cmd(f'nft delete rule ip6 raw dozenos_rpfilter handle {handle_search[1]}')
    ...
    def set_ipv6_source_validation(self, mode):
        ...
        nft_prefix = f'nft insert rule ip6 raw dozenos_rpfilter iifname "{self.ifname}"'
        if mode in ['strict', 'loose']:
            self._cmd(f"{nft_prefix} counter return")
        if mode == 'strict':
            self._cmd(f"{nft_prefix} fib saddr . iif oif 0 counter drop")

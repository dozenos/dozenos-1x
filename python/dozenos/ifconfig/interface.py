...
from dozenos.configdict import dict_merge
from dozenos.configdict import get_vlan_ids
...
from dozenos.utils.network import get_interface_address
from dozenos.utils.network import get_interface_namespace
...
from dozenos.utils.process import stop_systemd_unit
...
from dozenos.utils.assertion import assert_boolean
from dozenos.utils.assertion import assert_list
from dozenos.utils.assertion import assert_mac
from dozenos.utils.assertion import assert_mtu
from dozenos.utils.assertion import assert_positive
...
def _jmes_search(expression, data):
    ...
    jmespath at module scope would put it on the dozenos.ifconfig import path for
    no reason.
    """
    import jmespath
...
class Interface(Control):
    ...
    def __init__(self, ifname, **kargs):
        ...
        dozenos@dozenos# touch /tmp/dozenos.ifconfig.debug
    ...
    def _cleanup_mss_rules(self, table, ifname):
        ...
        results = self._cmdl(['nft', '-a', 'list', 'chain'] + table + ['DOZENOS_TCP_MSS']).split("\n")
        for line in results:
            if f'oifname "{ifname}"' in line:
                handle_search = re.search('handle (\d+)', line)
                if handle_search:
                    self._cmdl(['nft', 'delete', 'rule'] + table + ['DOZENOS_TCP_MSS', 'handle', handle_search[1]])
    ...
    def set_tcp_ipv4_mss(self, mss):
        ...
        nft_prefix = ['nft', 'add', 'rule', 'raw', 'DOZENOS_TCP_MSS']
    ...
    def set_tcp_ipv6_mss(self, mss):
        ...
        nft_prefix = ['nft', 'add', 'rule', 'ip6', 'raw', 'DOZENOS_TCP_MSS']
        base_cmd = ['oifname', f'"{self.ifname}"', 'tcp', 'flags', '&', '(syn|rst)', '==', 'syn']
        if mss == 'clamp-mss-to-pmtu':
            self._cmdl(nft_prefix + base_cmd + ['tcp', 'option', 'maxseg', 'size', 'set', 'rt', 'mtu'])
        elif int(mss) > 0:
    ...
    def _cleanup_ipv4_source_validation_rules(self, ifname):
        results = self._cmdl(['nft', '-a', 'list', 'chain', 'ip', 'raw', 'dozenos_rpfilter']).split("\n")
        for line in results:
            if f'iifname "{ifname}"' in line:
                handle_search = re.search('handle (\d+)', line)
                if handle_search:
                    self._cmdl(['nft', 'delete', 'rule', 'ip', 'raw', 'dozenos_rpfilter', 'handle', handle_search[1]])
    ...
    def set_ipv4_source_validation(self, mode):
        ...
        nft_prefix = ['nft', 'insert', 'rule', 'ip', 'raw', 'dozenos_rpfilter',
    ...
    def _cleanup_ipv6_source_validation_rules(self, ifname):
        results = self._cmdl(['nft', '-a', 'list', 'chain', 'ip6', 'raw', 'dozenos_rpfilter']).split("\n")
        for line in results:
            if f'iifname "{ifname}"' in line:
                handle_search = re.search('handle (\d+)', line)
                if handle_search:
                    self._cmdl(['nft', 'delete', 'rule', 'ip6', 'raw', 'dozenos_rpfilter', 'handle', handle_search[1]])
    ...
    def set_ipv6_source_validation(self, mode):
        ...
        nft_prefix = ['nft', 'insert', 'rule', 'ip6', 'raw', 'dozenos_rpfilter',
                      'iifname', f'"{self.ifname}"']
        if mode in ['strict', 'loose']:
            self._cmdl(nft_prefix + ['counter', 'return'])
        if mode == 'strict':
    ...
    def set_alias(self, ifalias=''):
        ...
        Example:
        >>> from dozenos.ifconfig import Interface
        >>> Interface('eth0').set_alias('DozenOS upstream interface')
    ...
    def set_dhcp(self, enable: bool, vrf_changed: bool=False):
        ...
        if enable and 'disable' not in self.config:
            if dict_search('dhcp_options.host_name', self.config) == None:
                ...
                # maybe change to dozenos-hostsd client ???
                hostname = 'dozenos'
    ...
    def set_eapol(self) -> None:
        ...
        if 'eapol' in self.config:
            # Imported here rather than at module scope: dozenos.pki imports the
            # whole cryptography stack, which every consumer of dozenos.ifconfig
            # would otherwise pay for on import. set_eapol() itself runs on
            # every ethernet/bond update, so keep it inside this guard too.
            # dozenos/config.py and dozenos/configverify.py defer dozenos.pki the same way.
            from dozenos.pki import find_chain
            from dozenos.pki import encode_certificate
            from dozenos.pki import load_certificate
            from dozenos.pki import wrap_private_key

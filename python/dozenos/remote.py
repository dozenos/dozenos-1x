...
from dozenos.progressbar import Progressbar
...
class HttpC:
    ...
    def _establish(self):
        ...
        session.headers.update({'User-Agent': 'DozenOS/' + get_version()})
        if self.username:
            ...
            with s.get(final_urlstring, stream=True, timeout=self.timeout) as r:
                ...
                with open(location, 'wb') as f:
                    if self.progressbar and size:
                        ...
                        #  files (like entire DozenOS images) don't occupy much memory.

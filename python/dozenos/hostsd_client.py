...
SOCKET_PATH = "ipc:///run/dozenos-hostsd/dozenos-hostsd.sock"

class DozenOSHostsdError(Exception):
    pass
...
class Client(object):
    def __init__(self):
        try:
            ...
            raise DozenOSHostsdError("Could not connect to dozenos-hostsd")
    ...
    def _communicate(self, msg):
        try:
            ...
            if 'error' in reply:
                raise DozenOSHostsdError(reply['error'])
            else:
                return reply["data"]
        except zmq.error.Again:
            raise DozenOSHostsdError("Could not connect to dozenos-hostsd")

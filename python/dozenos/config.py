...
"""
A library for reading DozenOS running config data.

This library is used internally by all config scripts of DozenOS,
but its API should be considered stable and safe to use
in user scripts.

Note that this module will not work outside DozenOS.

Node taxonomy
#############

There are multiple types of config tree nodes in DozenOS, each requires
...
DozenOS has two distinct modes: operational mode and configuration mode. When a user logins,
...
from dozenos.xml_ref import multi_to_list
from dozenos.xml_ref import from_source
from dozenos.xml_ref import ext_dict_merge
from dozenos.xml_ref import relative_defaults
from dozenos.utils.dict import get_sub_dict
from dozenos.utils.dict import mangle_dict_keys
from dozenos.utils.boot import boot_configuration_complete
from dozenos.utils.backend import vyconf_backend
from dozenos.configsource import ConfigSource
from dozenos.configsource import ConfigSourceSession
...
def config_dict_mangle_acme(name, cli_dict):
    ...
    """
    from dozenos.base import ConfigError
    ...
    try:
        ...
        if 'acme' in cli_dict:
            tmp = read_file(f'{dozenos_certbot_dir}/live/{name}/cert.pem')
            tmp = load_certificate(tmp, wrap_tags=False)
            cert_base64 = "".join(encode_certificate(tmp).strip().split("\n")[1:-1])

            tmp = read_file(f'{dozenos_certbot_dir}/live/{name}/privkey.pem')
...
class Config(object):
    ...
    def _make_path(self, path):
        ...
        # libdozenosconfig paths are lists, but since node names cannot contain whitespace,
    ...
    def exists(self, path):
        ...
        if self._session_config.exists(self._make_path(path)):
            ...
            # libdozenosconfig exists() works only for _nodes_, not _values_
            ...
            except dozenos.configtree.ConfigTreeError:
        ...
        if with_pki and conf_dict:
            ...
            if pki_dict:
                if 'certificate' in pki_dict:
                    from dozenos.defaults import directories
                    from dozenos.pki import acme_chain_ca_entry
                    from dozenos.pki import acme_chain_redundant
                    dozenos_certbot_dir = directories['certbot']
                    ...
                    for certificate in pki_dict['certificate']:
                        ...
                        if leaf_cert and 'acme' in cert_conf and not acme_chain_redundant(
                            ...
                            for autochain_name, chain_entry in acme_chain_ca_entry(
                                    dozenos_certbot_dir, certificate).items():

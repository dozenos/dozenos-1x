...
from dozenos.utils.permission import chmod
...
# outside of live DozenOS systems.
...
def bytes_to_human(bytes, initial_exponent=0, precision=2):
    ...
    from dozenos.utils.convert import bytes_to_human
...
def human_to_bytes(value):
    ...
    from dozenos.utils.convert import human_to_bytes
...
def get_dhcp_router(interface):
    ...
    /etc/dhcp/dhclient-exit-hooks.d/03-dozenos-dhclient-hook. It is intentionally
...
def nft_action(dozenos_action):
    if dozenos_action == 'accept':
        return 'return'
    return dozenos_action

@register_filter('nft_rule')
def nft_rule(rule_conf, fw_hook, fw_name, rule_id, ip_name='ip'):
    from dozenos.firewall import parse_rule
...
def nft_nested_group(out_list, includes, groups, key):
    if not dozenos_defined(out_list):
        out_list = []
...
def nat_rule(rule_conf, rule_id, nat_type, ipv6=False):
    from dozenos.nat import parse_nat_rule
...
def nat_static_rule(rule_conf, rule_id, nat_type):
    from dozenos.nat import parse_nat_static_rule
...
def conntrack_rule(rule_conf, rule_id, action, ipv6=False):
    ...
    if tcp_flags and action != 'timeout':
        from dozenos.firewall import parse_tcp_flags
...
def wlb_nft_rule(rule_conf, rule_id, local=False, exclude=False, limit=False, weight=None, health_state=None, action=None, restore_mark=False):
    from dozenos.wanloadbalance import nft_rule as wlb_nft_rule
...
def range_to_regex(num_range):
    """Convert range of numbers or list of ranges
       ...
       '(1[1-2]|1[4-5])'
    """
    from dozenos.range_regex import range_to_regex
...
def kea_client_class_json(client_classes):
    from dozenos.kea import kea_build_client_class_test
    from json import dumps
    out = []
...
def kea_dynamic_dns_update_main_json(config):
    from dozenos.kea import kea_parse_ddns_settings
    from json import dumps
...
def kea_dynamic_dns_update_tsig_key_json(config):
    from dozenos.kea import kea_parse_tsig_algo
    from json import dumps
    out = []
...
def kea_shared_network_json(shared_networks):
    from dozenos.kea import kea_parse_options
    from dozenos.kea import kea_parse_subnet
    from dozenos.kea import kea_parse_ddns_settings
    from json import dumps
    out = []
...
def kea6_shared_network_json(shared_networks):
    from dozenos.kea import kea6_parse_options
    from dozenos.kea import kea6_parse_subnet
    from json import dumps
    out = []
...
@register_test('dozenos_defined')
def dozenos_defined(value, test_value=None, var_type=None):
    """
    Jinja2 plugin to test if a variable is defined and not none - dozenos_defined
...
def get_default_port(service):
    """
    Jinja2 plugin to retrieve common service port number from dozenos.defaults
    ...
    if service not in internal_ports:
        raise RuntimeError(f'Service "{service}" not found in internal ' \
                           'dozenos.defaults.internal_ports dict!')
...
def get_default_config_file(filename):
    ...
    dozenos.defaults class from a Jinja2 template. This removes the need to
    ...
    if filename not in config_files:
        raise RuntimeError(f'Configuration file "{filename}" not found in '\
                           'internal dozenos.defaults.config_files dict!')

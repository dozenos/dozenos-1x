...
def _fromenv(flag):
    ...
    The presence of the environment DOZENOS_TEST_DEBUG (uppercase) enables it

    return empty string if not
    return content of env value it is
    """

    flagname = f'DOZENOS_{flag.upper()}_DEBUG'
...
def _fromfile(flag):
    ...
    For a given debug flag named "test"
    The presence of the file dozenos.test.debug (all lowercase) enables it
    ...
    for folder in ('/tmp', '/config'):
        flagfile = f'{folder}/dozenos.{flag}.debug'
...
def _contentenv(flag):
    return os.environ.get(f'DOZENOS_{flag.upper()}_DEBUG', '').strip()

...
def translate_exec(s: str) -> str:
    s = s.replace('${dozenos_op_scripts_dir}', directories['op_mode'])
    s = s.replace('${dozenos_libexec_dir}', directories['base'])
    return s
...
def translate_op_script(s: str) -> str:
    s = s.replace('${dozenos_completion_dir}', directories['completion_dir'])
    s = s.replace('${dozenos_op_scripts_dir}', directories['op_mode'])
    return s
...
def main():
    ...
    with open(op_ref_cache, 'w') as f:
        f.write('from dozenos.xml_ref.op_definition import NodeData\n')

...
from dozenos.xml_ref import op_definition

if TYPE_CHECKING:
    from dozenos.config import ConfigDict
...
def load_reference(cache=[]):
    ...
    try:
        from dozenos.xml_ref.cache import reference
    except Exception:
...
def load_op_reference(op_cache=[]):
    ...
    try:
        from dozenos.xml_ref.op_cache import op_reference
    except Exception:

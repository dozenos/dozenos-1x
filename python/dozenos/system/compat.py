...
def find_versions(menu_entries: list) -> list:
    """Find unique DozenOS versions from menu entries
    ...
    """
    versions = []
    for dozenos_ver in menu_entries:
        versions.append(dozenos_ver.get('version'))
    # remove duplicates
...
def filter_unparsed(grub_path: str) -> str:
    """Find currently installed DozenOS version
...
def prune_dozenos_versions(root_dir: str = '') -> None:
    """Delete dozenos-versions files of registered images subsequently deleted
    ...
    version_files = Path(f'{root_dir}/{grub.GRUB_DIR_DOZENOS_VERS}').glob('*.cfg')
...
def update_cfg_ver(root_dir:str = '') -> int:
    ...
    prune_dozenos_versions(root_dir)
...
def grub_cfg_fields(root_dir: str = '') -> dict:
    ...
    grub_cfg_main = f'{root_dir}/{grub.GRUB_CFG_MAIN}'
    grub_vars = f'{root_dir}/{grub.CFG_DOZENOS_VARS}'

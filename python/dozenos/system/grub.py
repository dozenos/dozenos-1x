...
GRUB_DIR_DOZENOS: str = f'{GRUB_DIR_MAIN}/grub.cfg.d'
CFG_DOZENOS_HEADER: str = f'{GRUB_DIR_DOZENOS}/00-dozenos-header.cfg'
CFG_DOZENOS_MODULES: str = f'{GRUB_DIR_DOZENOS}/10-dozenos-modules-autoload.cfg'
CFG_DOZENOS_VARS: str = f'{GRUB_DIR_DOZENOS}/20-dozenos-defaults-autoload.cfg'
CFG_DOZENOS_COMMON: str = f'{GRUB_DIR_DOZENOS}/25-dozenos-common-autoload.cfg'
CFG_DOZENOS_PLATFORM: str = f'{GRUB_DIR_DOZENOS}/30-dozenos-platform-autoload.cfg'
CFG_DOZENOS_MENU: str = f'{GRUB_DIR_DOZENOS}/40-dozenos-menu-autoload.cfg'
CFG_DOZENOS_OPTIONS: str = f'{GRUB_DIR_DOZENOS}/50-dozenos-options.cfg'
GRUB_DIR_DOZENOS_VERS: str = f'{GRUB_DIR_DOZENOS}/dozenos-versions'

TMPL_DOZENOS_VERSION: str = 'grub/grub_dozenos_version.j2'
...
BOOT_OPTS_STEM: str = 'boot=live rootdelay=5 noautologin net.ifnames=0 biosdevname=0 dozenos-union=/boot/'
...
def install(drive_path: str, boot_dir: str, efi_dir: str, id: str = 'DozenOS', chroot : str = "") -> None:
...
def version_add(version_name: str,
    ...
    """Add a new DozenOS version to GRUB loader configuration

    Args:
        dozenos_version (str): DozenOS version name
    ...
    version_config: str = f'{root_dir}/{GRUB_DIR_DOZENOS_VERS}/{version_name}.cfg'
    render(
        version_config, TMPL_DOZENOS_VERSION, {
...
def version_del(dozenos_version: str, root_dir: str = '') -> None:
    """Delete a DozenOS version from GRUB loader configuration
    ...
    if not root_dir:
        root_dir = disk.find_persistence()
    version_config: str = f'{root_dir}/{GRUB_DIR_DOZENOS_VERS}/{dozenos_version}.cfg'
    Path(version_config).unlink(missing_ok=True)


def version_list(root_dir: str = '') -> list[str]:
    """Generate a list with installed DozenOS versions
    ...
    if not root_dir:
        root_dir = disk.find_persistence()
    versions_files = Path(f'{root_dir}/{GRUB_DIR_DOZENOS_VERS}').glob('*.cfg')
...
def get_cfg_ver(root_dir: str = '') -> int:
    ...
    cfg_ver: str = vars_read(f'{root_dir}/{CFG_DOZENOS_HEADER}').get(
            'DOZENOS_CFG_VER')
    if cfg_ver:
...
def write_cfg_ver(cfg_ver: int, root_dir: str = '') -> None:
    ...
    vars_file: str = f'{root_dir}/{CFG_DOZENOS_HEADER}'
    vars_current: dict[str, str] = vars_read(vars_file)
    vars_current['DOZENOS_CFG_VER'] = str(cfg_ver)
...
def set_default(version_name: str, root_dir: str = '') -> None:
    ...
    vars_file = f'{root_dir}/{CFG_DOZENOS_VARS}'
...
def common_write(root_dir: str = '', grub_common: dict[str, str] = {}) -> None:
    ...
    if not root_dir:
        root_dir = disk.find_persistence()
    common_config = f'{root_dir}/{CFG_DOZENOS_COMMON}'
...
def create_structure(root_dir: str = '') -> None:
    ...
    Path(f'{root_dir}/{GRUB_DIR_DOZENOS_VERS}').mkdir(parents=True, exist_ok=True)
...
def set_console_type(console_type: str, root_dir: str = '') -> None:
    ...
    vars_file: str = f'{root_dir}/{CFG_DOZENOS_VARS}'
    vars_current: dict[str, str] = vars_read(vars_file)

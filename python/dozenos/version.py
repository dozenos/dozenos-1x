...
"""
DozenOS version data access library.

DozenOS stores its version data, which include the version number and some
...
Example of the version data dict::
  {
   'built_by': 'autobuild@dozenos.local',
...
version_file = os.path.join(dozenos.defaults.directories['data'], 'version.json')
...
def get_full_version_data(fname=version_file):
    # Imported here rather than at module scope: dozenos.system.image pulls in
    # dozenos.system.grub -> dozenos.template -> jinja2, which every consumer of
    # dozenos.version would otherwise pay for on import.
    from dozenos.system.image import is_live_boot
    ...
    [
       {
        ...
        "image":"dozenos-rolling-latest.iso",
        "latest":true,
        "lts":false,
        "release_date":"2022-09-06",
        "release_train":"sagitta",
        "url":"http://xxx/rolling/current/dozenos-rolling-latest.iso",
        "version":"dozenos-1.4-rolling-202209060217"
      }
    ]
    """

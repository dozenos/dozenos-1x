...
let generate dirname =
  let prefix = "dozenosconfig" in
  let path basename = Filename.concat dirname basename in
  let ml_fd = open_out (path "dozenosconfig_bindings.ml") in
  let c_fd = open_out (path "dozenosconfig.c") in
  let h_fd = open_out (path "dozenosconfig.h") in

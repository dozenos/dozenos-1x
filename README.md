# dozenos-1x: DozenOS command definitions, configuration scripts, and data

DozenOS 1.1.x had its codebase split into way too many submodules for no good
reason, which made it hard to navigate or write meaningful changelogs. As the
code undergoes rewrite in the new style in DozenOS 1.2.0+, we consolidate the
rewritten code in this package.

If you just want to build a DozenOS image, the repository you want is
[dozenos-build](https://github.com/dozenos/dozenos-build). If you also want to contribute
to DozenOS, read on.
...
* Use separate functions for retrieving configuration data, validating it, and
  ...
  [documentation](https://docs.dozenos.io/en/latest/contributing/development.html#python)
...
Runtime tests are executed by the CI system on a running DozenOS instance inside
QEMU. The testcases can be found inside the smoketest subdirectory which will
be placed into the dozenos-1x-smoketest package.


### Thanks to all the people who already contributed!

<a href="https://github.com/dozenos/dozenos-1x/graphs/contributors">
  <img src="https://contributors-img.web.app/image?repo=dozenos/dozenos-1x" />
</a>

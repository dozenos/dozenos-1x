...
from dozenos.utils.network import get_nft_vrf_zone_mapping
from dozenos.xml_ref import cli_defined
...
class BasicInterfaceTest:
    class TestCase(DozenOSUnitTestSHIM.TestCase):
        ...
        def test_dhcp_client_options(self):
            ...
            client_id = 'DozenOS-router'
            distance = '100'
            hostname = 'dozenos'
            vendor_class_id = 'dozenos-vendor'
            user_class = 'dozenos'
        ...
        def test_vif_8021q_mtu_limits(self):
            ...
            # Some drivers are limited to an MTU of 1500 byte - e.g. when DozenOS
        ...
        def test_vif_8021q_lower_up_down(self):
            # Testcase for https://dozenos.dev/T3349
            if not self._test_vlan:
                self.skipTest(MSG_TESTCASE_UNSUPPORTED)
...
#                           self.assertTrue(is_intf_addr_assigned(vif, address))
        ...
        def test_interface_ip_options(self):
            ...
            for interface in self._interfaces:
                if cli_defined(self._base_path + ['ip'], 'adjust-mss'):
                    ...
                    out = cmdl(['nft', 'list', 'chain', 'raw', 'DOZENOS_TCP_MSS'], sudo=True)
                ...
                if cli_defined(self._base_path + ['ip'], 'source-validation'):
                    ...
                    out = cmdl(['nft', 'list', 'chain', 'ip', 'raw', 'dozenos_rpfilter'], sudo=True)
                    for line in out.splitlines():
                        if line.startswith(base_options):
        ...
        def test_interface_ipv6_options(self):
            ...
            for interface in self._interfaces:
                ...
                if cli_defined(self._base_path + ['ipv6'], 'adjust-mss'):
                    base_options = f'oifname "{interface}"'
                    out = cmdl(['nft', 'list', 'chain', 'ip6', 'raw', 'DOZENOS_TCP_MSS'], sudo=True)
                    for line in out.splitlines():
                        if line.startswith(base_options):
                            self.assertIn(f'tcp option maxseg size set {mss}', line)
                ...
                if cli_defined(self._base_path + ['ipv6'], 'source-validation'):
                    base_options = f'iifname "{interface}"'
                    out = cmdl(['nft', 'list', 'chain', 'ip6', 'raw', 'dozenos_rpfilter'], sudo=True)
                    for line in out.splitlines():
                        if line.startswith(base_options):

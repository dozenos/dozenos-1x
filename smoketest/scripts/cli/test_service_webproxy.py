...
class TestServiceWebProxy(DozenOSUnitTestSHIM.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_03_ldap_proxy_auth(self):
        auth_children = '20'
        cred_ttl = '120'
        realm = 'DozenOS Webproxy'
        ldap_base_dn = 'DC=dozenos,DC=net'
        ldap_server = 'ldap.dozenos.net'
        ldap_bind_dn = f'CN=proxyuser,CN=Users,{ldap_base_dn}'
        ldap_password = 'DozenOS12345'
        ldap_attr = 'cn'
        ldap_filter = '(cn=%s)'
    ...
    def test_05_basic_squidguard(self):
        ...
        default_cache = '100'
        local_block = ['192.0.0.1', '10.0.0.1', 'block.dozenos.net']
        local_block_url = ['foo.com/bar.html', 'bar.com/foo.htm']
        local_block_pattern = ['porn', 'cisco', 'juniper']
        local_ok = ['10.0.0.0', 'dozenos.net']
        local_ok_url = ['dozenos.net', 'dozenos.io']

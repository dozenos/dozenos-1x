...
busybox_image_path = '/usr/share/dozenos/busybox-stable.tar'
...
class TestContainer(DozenOSUnitTestSHIM.TestCase):
    @classmethod
    def setUpClass(cls):
        super(TestContainer, cls).setUpClass()

        # Load image for smoketest provided in dozenos-1x-smoketest
    ...
    def tearDown(self):
        ...
        units = glob.glob('/run/containers/systemd/dozenos*')
        self.assertEqual(units, [])
        # always forward to base class
        super().tearDown()

    def is_running(self, name):
        command = ['systemctl', 'show', f'dozenos-container-{name}', '--property=ActiveState', '--value']
    ...
    def test_network_types(self):
        ...
        for network in ['macvlan1', 'macvlan2', 'macvlan3', 'bridge1', 'bridge2']:
            systemd_unit = f'dozenos-{network}-network.service'

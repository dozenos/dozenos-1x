...
from dozenos.netlink.ovpn import get_ovpn_mode
from dozenos.netlink.ovpn import OVPN_MODE_MP
from dozenos.netlink.ovpn import OVPN_MODE_P2P
from dozenos.template import address_from_cidr
from dozenos.template import inc_ip
from dozenos.template import last_host_address
from dozenos.template import netmask_from_cidr
...
class TestInterfacesOpenVPN(DozenOSUnitTestSHIM.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_openvpn_client_verify(self):
        ...
        self.cli_set(path + ['authentication', 'username', 'dozenos'])
        with self.assertRaises(ConfigSessionError):
            self.cli_commit()
        self.cli_set(path + ['authentication', 'password', 'dozenos'])
    ...
    def test_openvpn_client_dco(self):
        ...
        # the interface, so this is the one case where DozenOS is definitively the
    ...
    def test_openvpn_dco_peer_reaches_the_kernel(self):
        # DozenOS creates the "ovpn" device itself whenever the daemon cannot -
        ...
        # the client first, with nothing listening - DozenOS makes the device
        ...
        # the side that had to adopt a device DozenOS made for it
        self.assertGreater(
            self._peers(client),
            0,
            f'{client} has no peer after adopting the device DozenOS made\n'
    ...
    _peer_probe = (
        'import sys\n'
        'from dozenos.netlink.ovpn import get_ovpn_peers\n'
        'try:\n'
    ...
    def test_openvpn_client_dco_raw_option(self):
        # A raw option no longer keeps DozenOS from creating the device: the
    ...
    def assertDcoDataPath(self, interface, multipoint=True):
        ...
        # DozenOS creates the device itself, so finding it in the right mode does
    ...
    def test_openvpn_server_dco_raw_option(self):
        ...
        # DozenOS creates the device either way - verify() turns away the options

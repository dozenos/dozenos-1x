...
class VRFTest(DozenOSUnitTestSHIM.TestCase):
    _interfaces = []
    ...
    def tearDown(self):
        ...
        # DozenOS can not remove a veth pair with one end in another namespace
    ...
    def test_vrf_vni_and_table_id(self):
        ...
        table = base_table
        iproute2_config = read_file('/etc/iproute2/rt_tables.d/dozenos-vrf.conf')
        for vrf in vrfs:
            description = f'DozenOS-VRF-{vrf}'
            ...
            #
            # # id       vrf name         comment
            # 1000       red              # DozenOS-VRF-red
            # 1001       green            # DozenOS-VRF-green
            #  ...
    ...
    def test_vrf_ipv6_protocol_route_map(self):
        ...
        for vrf in vrfs:
            ...
            self.assertIn(f'vrf {vrf}', frrconfig)
            for protocol in v6_protocols:
                # DozenOS and FRR use a different name for OSPFv3 (IPv6)
                if protocol == 'ospfv3':
                    protocol = 'ospf6'
    ...
    def test_vrf_conntrack_route_leak(self):
        ...
        # not have. DozenOS can not build a veth pair straddling a namespace

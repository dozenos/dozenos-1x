...
from dozenos.utils.file import chmod_755
...
class TestLoadBalancingWan(DozenOSUnitTestSHIM.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def tearDown(self):
        ...
        for chain in removed_chains:
            self.verify_nftables_chain_exists('ip dozenos_wanloadbalance', chain, inverse=True)
	...
	chain wlb_mangle_prerouting {
        ...
        nat_wanloadbalance = """table ip dozenos_wanloadbalance {
	chain wlb_nat_postrouting {
        ...
        mangle_prerouting_limit = """table ip dozenos_wanloadbalance {
	chain wlb_mangle_prerouting {
		type filter hook prerouting priority mangle; policy accept;
...
"""
        ...
        self.verify_nftables_chain(nftables_search, 'ip dozenos_wanloadbalance', 'wlb_mangle_prerouting')
    ...
    def test_3_or_more_interfaces_in_rule(self):
        ...
        def check_wlb_status():
            rc, wlb_status = rc_cmd('nft list chain ip dozenos_wanloadbalance wlb_mangle_prerouting')
            if rc != 0:
                return False

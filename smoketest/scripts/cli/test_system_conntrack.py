...
def get_logger_config():
    return read_json('/run/dozenos-conntrack-logger.conf')
...
class TestSystemConntrack(DozenOSUnitTestSHIM.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        for module, module_options in modules.items():
            ...
            if 'nftables' in module_options:
                for rule in module_options['nftables']:
                    self.assertTrue(find_nftables_rule('ip dozenos_conntrack', 'DOZENOS_CT_HELPER', [rule]) != None)
    ...
    def test_conntrack_ignore(self):
        ...
        self.verify_nftables(nftables_search, 'ip dozenos_conntrack')
        self.verify_nftables(nftables6_search, 'ip6 dozenos_conntrack')

        self.cli_delete(['firewall'])

    def test_conntrack_timeout_custom(self):
        # No timeout rules configured yet, so there should be no DOZENOS_CT_TIMEOUT chain or timeout base chains
        ...
        self.verify_nftables_chain_exists(
            'ip dozenos_conntrack', 'DOZENOS_CT_TIMEOUT', inverse=True
        )

        self.verify_nftables_chain_exists(
            'ip6 dozenos_conntrack', 'DOZENOS_CT_TIMEOUT', inverse=True
        )

        self.verify_nftables_chain_exists(
            'ip dozenos_conntrack', 'PREROUTING_CT_TIMEOUT', inverse=True
        )

        self.verify_nftables_chain_exists(
            'ip6 dozenos_conntrack', 'PREROUTING_CT_TIMEOUT', inverse=True
        )

        self.verify_nftables_chain_exists(
            'ip dozenos_conntrack', 'OUTPUT_CT_TIMEOUT', inverse=True
        )

        self.verify_nftables_chain_exists(
            'ip6 dozenos_conntrack', 'OUTPUT_CT_TIMEOUT', inverse=True
        )
        ...
        self.verify_nftables_chain_exists('ip dozenos_conntrack', 'DOZENOS_CT_TIMEOUT')

        self.verify_nftables_chain_exists(
            'ip6 dozenos_conntrack', 'DOZENOS_CT_TIMEOUT', inverse=True
        )

        self.verify_nftables_chain_exists('ip dozenos_conntrack', 'PREROUTING_CT_TIMEOUT')

        self.verify_nftables_chain_exists(
            'ip6 dozenos_conntrack', 'PREROUTING_CT_TIMEOUT', inverse=True
        )

        self.verify_nftables_chain_exists('ip dozenos_conntrack', 'OUTPUT_CT_TIMEOUT')
        ...
        self.verify_nftables_chain(
            prerouting_timeout_chain, 'ip dozenos_conntrack', 'PREROUTING_CT_TIMEOUT'
        )

        self.verify_nftables_chain(
            output_timeout_chain, 'ip dozenos_conntrack', 'OUTPUT_CT_TIMEOUT'
        )

        # Verify that IPv4 base chain priority is a number greater than -200
        if not chain_priority_conntrack_compatible(
            'ip dozenos_conntrack', 'PREROUTING_CT_TIMEOUT', 'filter', 'prerouting'
        ):
            self.fail(
        ...
        if not chain_priority_conntrack_compatible(
            'ip dozenos_conntrack', 'OUTPUT_CT_TIMEOUT', 'filter', 'output'
        ):
            self.fail(
        ...
        self.verify_nftables_chain_exists('ip dozenos_conntrack', 'DOZENOS_CT_TIMEOUT')
        self.verify_nftables_chain_exists('ip6 dozenos_conntrack', 'DOZENOS_CT_TIMEOUT')
        self.verify_nftables_chain_exists('ip dozenos_conntrack', 'PREROUTING_CT_TIMEOUT')
        self.verify_nftables_chain_exists('ip6 dozenos_conntrack', 'PREROUTING_CT_TIMEOUT')
        self.verify_nftables_chain_exists('ip dozenos_conntrack', 'OUTPUT_CT_TIMEOUT')
        self.verify_nftables_chain_exists('ip6 dozenos_conntrack', 'OUTPUT_CT_TIMEOUT')
        ...
        self.verify_nftables_chain(
            prerouting_timeout_chain, 'ip6 dozenos_conntrack', 'PREROUTING_CT_TIMEOUT'
        )
        ...
        self.verify_nftables_chain(
            output_timeout_chain, 'ip6 dozenos_conntrack', 'OUTPUT_CT_TIMEOUT'
        )

        # Verify that IPv6 base chain priority is a number greater than -200
        if not chain_priority_conntrack_compatible(
            'ip6 dozenos_conntrack', 'PREROUTING_CT_TIMEOUT', 'filter', 'prerouting'
        ):
            self.fail(
                'PREROUTING_CT_TIMEOUT base chain must have priority > -200 to read and update conntrack entries'
            )
        ...
        if not chain_priority_conntrack_compatible(
            'ip6 dozenos_conntrack', 'OUTPUT_CT_TIMEOUT', 'filter', 'output'
        ):
            self.fail(
                'OUTPUT_CT_TIMEOUT base chain must have priority > -200 to read and update conntrack entries'
            )

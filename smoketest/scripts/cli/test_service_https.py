...
from base_dozenostest_shim import ignore_warning
...
class TestHTTPSService(DozenOSUnitTestSHIM.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_listen_address_vrf(self):
        ...
        self.cli_set(
            base_path + ['api', 'keys', 'id', 'key-01', 'key', 'MySuperSecretDozenOS']
        )
    ...
    def test_api_auth(self):
        ...
        r = request('POST', url, verify=False, headers=headers, data=payload)
        response = r.json()
        dozenos_user_exists = 'dozenos' in response.get('data', {}).get('user', {})
        self.assertTrue(
            dozenos_user_exists, "The 'dozenos' user does not exist in the response."
        )
        ...
        mutation = """
        mutation {
          AuthToken (data: {username: "dozenos", password: "dozenos"}) {
            success
            errors
            data {
              result

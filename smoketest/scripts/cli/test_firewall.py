...
class TestFirewall(DozenOSUnitTestSHIM.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def tearDown(self):
        ...
        self.verify_nftables(nftables_search, 'ip dozenos_filter', inverse=True)
    ...
    def test_geoip(self):
        ...
        self.verify_nftables(nftables_search, 'ip dozenos_filter', args='-t')
    ...
    def test_fib_type(self):
        ...
        self.verify_nftables_chain(
            nftables_search_v4, 'ip dozenos_filter', 'NAME_smoketest'
        )
        self.verify_nftables_chain(
            nftables_search_v6, 'ip6 dozenos_filter', 'NAME6_smoketest'
        )
        ...
        self.verify_nftables_chain(
            nftables_search_v4_raw, 'ip dozenos_filter', 'DOZENOS_PREROUTING_raw'
        )
        self.verify_nftables_chain(
            nftables_search_v6_raw, 'ip6 dozenos_filter', 'DOZENOS_IPV6_PREROUTING_raw'
        )
    ...
    def test_groups(self):
        ...
        self.wait_for_domain_resolver('ip dozenos_filter', 'D_smoketest_domain', '192.0.2.5')
    ...
    def test_ipv4_basic_rules(self):
        ...
        nftables_search = [
            ...
            ['chain DOZENOS_OUTPUT_raw'],
            ['type filter hook output priority raw; policy accept;'],
            ['udp', 'accept'],
            ['OUT-raw default-action drop', 'drop'],
            ['chain DOZENOS_PREROUTING_raw'],
    ...
    def test_ipv6_basic_rules(self):
        ...
        nftables_search = [
            ['chain DOZENOS_IPV6_FORWARD_filter'],
            ['type filter hook forward priority filter; policy accept;'],
            ...
            ['jump DOZENOS_STATE_POLICY6'],
            ['chain DOZENOS_STATE_POLICY6'],
    ...
    def test_ipv4_global_state(self):
        ...
        nftables_search = [
            ['jump DOZENOS_STATE_POLICY'],
            ['chain DOZENOS_STATE_POLICY'],
            ['jump DOZENOS_STATE_POLICY_FORWARD'],
            ['chain DOZENOS_STATE_POLICY_FORWARD'],
            ['flow add @DOZENOS_FLOWTABLE_smoketest'],
            ['ct state established', 'accept'],
            ['ct state invalid', 'drop'],
            ['ct state related', 'accept']
        ]
        ...
        self.verify_nftables([['ct state invalid', 'reject']], 'ip dozenos_filter')
        self.verify_nftables([['ct state invalid', 'drop']], 'bridge dozenos_filter')

        # Check conntrack is enabled from state-policy
        self.verify_nftables_chain([['accept']], 'ip dozenos_conntrack', 'FW_CONNTRACK')
        self.verify_nftables_chain([['accept']], 'ip6 dozenos_conntrack', 'FW_CONNTRACK')
    ...
    def test_ipv4_state_and_status_rules(self):
        ...
        # Check conntrack
        self.verify_nftables_chain([['accept']], 'ip dozenos_conntrack', 'FW_CONNTRACK')
        self.verify_nftables_chain([['return']], 'ip6 dozenos_conntrack', 'FW_CONNTRACK')
    ...
    def test_bridge_firewall(self):
        ...
        nftables_search = [
            ...
            ['chain DOZENOS_PREROUTING_filter'],
            ['type filter hook prerouting priority filter; policy accept;'],
            ['ip6 daddr @A6_AGV6', 'notrack'],
            ['ether type arp', 'accept']
        ]

        self.verify_nftables(nftables_search, 'bridge dozenos_filter')
    ...
    def test_source_validation(self):
        ...
        self.verify_nftables_chain(nftables_strict_search, 'ip raw', 'dozenos_global_rpfilter')
        self.verify_nftables_chain(nftables_strict_search, 'ip6 raw', 'dozenos_global_rpfilter')
        ...
        self.verify_nftables_chain(nftables_loose_search, 'ip raw', 'dozenos_global_rpfilter')
        self.verify_nftables_chain(nftables_loose_search, 'ip6 raw', 'dozenos_global_rpfilter')
...
### Zone
    def test_zone_basic(self):
        ...
        nftables_search = [
            ['chain DOZENOS_ZONE_FORWARD'],
            ['type filter hook forward priority filter + 1'],
            ['chain DOZENOS_ZONE_OUTPUT'],
            ['type filter hook output priority filter + 1'],
            ['chain DOZENOS_ZONE_LOCAL'],
        ...
        self.verify_nftables(nftables_search, 'ip dozenos_filter')
        self.verify_nftables(nftables_search_v6, 'ip6 dozenos_filter')
    ...
    def test_zone_with_default_firewall(self):
        ...
        self.verify_nftables_chain_exists('ip dozenos_filter', 'VZONE_smoketest-eth0')
        self.verify_nftables_chain(smoketest_eth0_search, 'ip dozenos_filter', 'VZONE_smoketest-eth0')

        smoketest_eth1_search = [
            ['jump NAME_smoketest-default']
        ]
        self.verify_nftables_chain_exists('ip dozenos_filter', 'VZONE_smoketest-eth1')
        self.verify_nftables_chain(smoketest_eth1_search, 'ip dozenos_filter', 'VZONE_smoketest-eth1')

        self.verify_nftables_chain_exists('ip dozenos_filter', 'VZONE_smoketest-eth2')

        smoketest_local_in_search = [
            ['iifname "eth0"', 'jump NAME_smoketest'],
            ['jump NAME_smoketest-default'],
        ]
        self.verify_nftables_chain_exists('ip dozenos_filter', 'VZONE_smoketest-local_IN')
        self.verify_nftables_chain(smoketest_local_in_search, 'ip dozenos_filter', 'VZONE_smoketest-local_IN')

        smoketest_local_out_search = [
            ['oifname "eth0"', 'jump NAME_smoketest'],
            ['oifname "eth1"', 'jump NAME_smoketest-default']
        ]
        self.verify_nftables_chain_exists('ip dozenos_filter', 'VZONE_smoketest-local_OUT')
        self.verify_nftables_chain(smoketest_local_out_search, 'ip dozenos_filter', 'VZONE_smoketest-local_OUT')
    ...
    def test_wildcard_interfaces(self):
        ...
        self.verify_nftables(wc_interfaces, 'ip dozenos_filter')
    ...
    def test_flow_offload(self):
        ...
        nftables_search = [
            ['flowtable DOZENOS_FLOWTABLE_smoketest'],
            ['hook ingress priority filter'],
            ['devices = { eth0.10 }'],
            ['ct state { established, related }', 'meta l4proto { tcp, udp }', 'flow add @DOZENOS_FLOWTABLE_smoketest'],
        ]
    ...
    def test_gre_match(self):
        ...
        self.verify_nftables(nftables_search_v4, 'ip dozenos_filter')
        self.verify_nftables(nftables_search_v6, 'ip6 dozenos_filter')
    ...
    def test_remote_group(self):
        ...
        self.verify_nftables(nftables_v4_search, 'ip dozenos_filter')
        ...
        self.verify_nftables(nftables_v6_search, 'ip6 dozenos_filter')
    ...
    def test_disable_conntrack_per_chain(self):
        ...
        nftables_search = [
            ['DOZENOS_DISABLE_CONNTRACK_INP_FWD'],
            ['DOZENOS_DISABLE_CONNTRACK_OUT'],
        ...
        nftables_search = [
            ['DOZENOS_DISABLE_CONNTRACK_INP_FWD_V6'],
            ['DOZENOS_DISABLE_CONNTRACK_OUT_V6'],
            ['fib daddr . iif type unicast notrack counter'],
            ['fib daddr . iif type local notrack counter ']
        ]

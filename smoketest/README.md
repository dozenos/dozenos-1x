...
Smoketests execute predefined DozenOS CLI commands and verify that the
...
be run manually by simply executing the Python script on a live DozenOS system.

```
dozenos@dozenos:~$ /usr/libexec/dozenos/tests/smoke/cli/test_protocols_bgp.py
...
```
dozenos@dozenos:~$ /usr/libexec/dozenos/tests/smoke/cli/test_protocols_bgp.py -k test_bgp_02_neighbors
test_bgp_02_neighbors (__main__.TestProtocolsBGP) ... ok
...
The files in `smoketests/configs/` are real DozenOS configurations taken from
...
After assembling a DozenOS ISO image from dozenos-build repository, a `make testc`

...
.PHONY: all
all: clean copyright libdozenosconfig pylint interface_definitions op_mode_definitions test j2lint vyshim generate-configd-include-json generate-activation-scripts-json ocaml
...
pylint: interface_definitions
	@echo Running "pylint ..."
	@set -e; \
	PYTHONPATH="python/:smoketest/scripts/cli/" pylint --jobs 0 --errors-only $(shell git ls-files python/**/*.py src/conf_mode/*.py src/op_mode/*.py src/migration-scripts src/services/dozenos* smoketest/scripts); \

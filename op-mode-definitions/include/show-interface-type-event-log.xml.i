...
<node name="event-log">
  ...
  <command>journalctl --no-hostname --boot --unit dozenos-network-event-logger.service --grep "\b$4\b"</command>
  <children>
    <leafNode name="route">
      <properties>
        <help>Show log for route events</help>
      </properties>
      <command>journalctl --no-hostname --boot --unit dozenos-network-event-logger.service --grep "\b$4\b" | grep -i "\[$6\]"</command>
    </leafNode>
    <leafNode name="link">
      <properties>
